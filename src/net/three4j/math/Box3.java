...
//
	...
	public boolean satForAxes(double[] axes, Vector3 v0, Vector3 v1, Vector3 v2, Vector3 extents) {
		...
		for (int i = 0, j = axes.length - 3; i <= j; i += 3) {
			...
			// actual test, basically see if either of the most extreme of the triangle
			// points intersects r
			if (Math.max(-Math.max(p0, Math.max(p1, p2)), Math.min(p0, Math.max(p1, p2))) > r) {

				// points of the projected triangle are outside the projected half-length of the
				// aabb
