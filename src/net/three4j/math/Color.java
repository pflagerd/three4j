...
//"yellowgreen": 0x9ACD32 
		...
		if ( m.matches() ) {
			...
			switch ( name ) {
				...
				case "rgba":
					...
					color = Pattern.compile("/^(\\d+)\\s*,\\s*(\\d+)\\s*,\\s*(\\d+)\\s*(?:,\\s*(\\d*\\.?\\d+)\\s*)?$").matcher(components );
					if ( color.matches() ) {
				...
				case "hsla":
					...
					color = Pattern.compile("^(\\d*\\.?\\d+)\\s*,\\s*(\\d+)\\%\\s*,\\s*(\\d+)\\%\\s*(?:,\\s*(\\d*\\.?\\d+)\\s*)?$").matcher(components );
					if ( color.matches() ) {
	...
	public String getHexString() {
		...
		return ("000000" + s).substring(s.length() - 6);
	...
	public final static HashMap<String, Integer> NAMES = _colorKeywords;
